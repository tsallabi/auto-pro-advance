...
function getOrAddSeller() {
    ...
    if (!seller) {
        ...
        // Create Wallet
        db.prepare("INSERT INTO wallets (id, userId) VALUES (?, ?)").run('wallet-seller-sim-1', 'seller-sim-1');
    }
    return seller;
}
...
function ensureBuyers(count = 5) {
    ...
    for (let i = 1; i <= count; i++) {
        ...
        if (!buyer) {
            ...
            db.prepare("INSERT INTO wallets (id, userId, availableBalance) VALUES (?, ?, ?)")
                .run(`wallet-${id}`, id, 50000); // 50k deposit
